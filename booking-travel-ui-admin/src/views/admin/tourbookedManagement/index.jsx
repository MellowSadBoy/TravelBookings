...
import DevelopmentTable from "views/admin/tourbookedManagement/components/DevelopmentTable";

import { columnsDataDevelopment } from "views/admin/tourbookedManagement/variables/columnsData";
...
export default function Settings() {
  ...
  useEffect(() => {
    if (isReset) {
      ...
      httpServ.filterTourbooked(searchData).then((res) => {
  ...
  const handleSearch = (vl) => {
    const dataFilter = allData.filter((tour) => {
      return tour.tourName.toLowerCase().includes(vl.toLowerCase());
    });
    setData(dataFilter);
  };
