...
const DModal = ({
  ...
  const renderHeader = () => {
    if (isUpdate) {
      return <>CẬP NHẬT TOUR ĐÃ ĐẶT</>;
    }
    if (isDetail) {
      return <>CHI TIẾT TOUR ĐÃ ĐẶT</>;
    }
  ...
  useEffect(() => {
    ...
    setLoading(true);
    httpServ.getTourbookedById(idDetail ? idDetail : idUpdate).then((res) => {
      const { tour, tourBooked } = res.data;

      console.log("id tourbooked ", idDetail);

      console.log(
        "🚀 ~ file: Modal.js:111 ~ httpServ.getTourbookedById ~ tour:",
        tour
      );
      console.log(
        "🚀 ~ file: Modal.js:111 ~ httpServ.getTourbookedById ~ tourbooked:",
        tourBooked
      );
